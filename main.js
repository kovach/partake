  ...
  switch (operation.tag) {
    ...
    case "subQuery":
      for (let c of contexts) {
        ...
        console.log("!!", choices);
        //let choices = af(joins(db, body, [c.binding, 1])).map(([t, _]) => t); // todo
...
function fixStack(db, rules, trace, stack) {
  ...
  loop: while (stack.length > 0 && count++ < stepLimit) {
    ...
    switch (obj.tag) {
      ...
      case "line":
        ...
        if (initial) trace.push({ tag: "record", db: clone(db), name });
  ...
  trace.push({ tag: "record", db: clone(db), name: "end" });
}
...
function renderChoice(
  ...
  e.onmouseenter = () => {
    for (let tuple of used) {
      console.log("used", tuple);
      for (let elem of tupleRefs.get(str(tuple))) {
        console.log("elem", elem);
        elem.classList.add("hl");
      }
    }
    //for (let key in bindingDiff(parentBinding, binding))
    //  for (let elem of valRefs.get(binding[key])) elem.classList.add("hl");
  };
  e.onclick = () => {
...
function checkGlobalChoiceState() {
  ...
  let { db, rules, trace, stack, contexts, operations, name } =
  ...
  stack.push(mkLine({ contexts, operations, initial: false, name }));
...
window.onload = () => {
  ...
  resetDb();
  let trace = mkTrace();
  function go(operations) {
    return fixStack(db, rules, trace, [
      mkLine({ name: "repl", initial: true, contexts, operations }),
    ]);
  }
  go(line1);
  go(line2);
...
display line during evaluation
interesting query
